...
Reads:   skill_deterministic.csv, skill_probabilistic.csv  (same directory or parent)
...
def _find_data_dir():
    candidates = [os.path.dirname(HERE), HERE]
    for d in candidates:
        if os.path.isfile(os.path.join(d, 'skill_deterministic.csv')):
            return d
    raise FileNotFoundError("skill_deterministic.csv not found in candidate dirs")

data_dir = _find_data_dir()
print(f"Reading CSV data from: {data_dir}")
...
def fig2_pcc_regional_TP():
    ...
    models = ['FuXi', 'ECMWF', 'MME', 'Persistence']
...
def fig4_rmse_bias_ssr():
    ...
    all_models  = ['FuXi', 'ECMWF', 'MME', 'Persistence']
    ...
    fig.legend(h2, l2, loc='lower center', ncol=5,
               fontsize=8, framealpha=0.9, bbox_to_anchor=(0.5, -0.04))
    fig.tight_layout()
...
def fig5_msss_crpss_regional():
    vars_   = ['TP', 'Z500']
    metrics = [('msss_clim', det,  ['FuXi', 'ECMWF', 'MME'], 'MSSS vs Clim'),
               ('crpss_clim', prob, ['FuXi', 'ECMWF'],        'CRPSS vs Clim')]
