...
anchor, loss, seeds, and split. The attention candidate can choose a convex
...
def set_experiment_scope(
    *,
    ...
    if large_model:
        RESULTS_ROOT = (
            HERE / "results" / "fuxi_imd_attention_climatology_big_allweeks"
        )
        NORMAL = engine.Candidate(
            "normal_climo_model",
...
def build_climatology_features(
    ...
    *,
    ...
    centre_difference = float(
        np.nanmax(np.abs(bank[:, :, OFFSETS_DAYS.index(0)] - observations.weekly_climatology))
    )
    ...
    normalized_bank = (
        log_bank - climo_mean[None, :, None, None, None]
    ...
    if features.shape != (
        ...
        raise base.DataContractError(f"unexpected attentive feature shape: {features.shape}")
    normalization = dict(normalization)
    normalization["log_imd_climatology"] = normalization.pop(
    ...
    if member_summaries is not None:
        ...
        normalization["fuxi_member_summaries"] = {
            ...
            "definitions": {
                name: common.MEMBER_SUMMARY_DEFINITIONS[name]
                for name in member_names
            },
    ...
    if preserve_fuxi_context:
        ...
        if any(np.any(features[:, :, index, ~support] != 0.0) for index in support_limited):
            raise base.DataContractError(
...
def add_attention_comparison(
    ...
    pooled_skill = 100.0 * (normal_rmse.mean() - attention_rmse.mean()) / normal_rmse.mean()
    result["attention_rmse_skill_vs_normal_pooled_pct"] = np.nan
    result.loc[
        attention_row, "attention_rmse_skill_vs_normal_pooled_pct"
    ] = pooled_skill
    return result
...
def choose_model(selection: pd.DataFrame) -> tuple[str, float, str]:
    ...
    if (
        bool(attention.improves_every_validation_year)
        and bool(attention.attention_beats_normal_every_year)
    ):
        return (
            ATTENTION.name,
            float(attention.alpha),
            "Attention improved over both log-bias and the normal-climatology model in each validation year.",
        )
    ...
    if bool(attention.improves_every_validation_year):
        return (
            ATTENTION.name,
            float(attention.alpha),
            "Normal climatology failed the log-bias guard; attention improved over log-bias in both years.",
        )
    return "log_bias", 0.0, "Neither learned candidate improved over log-bias in both validation years."


def plot_validation(
    scan: pd.DataFrame, selection: pd.DataFrame, output: Path
) -> None:
    ...
    skills = [float(getattr(attention, f"attention_rmse_skill_vs_normal_{year}_pct")) for year in years]
...
def plot_imd_skill_by_lead(
    ...
    *,
    ...
    context = "SMOKE CHECK" if smoke else f"IMD verification; reused 2020–2021 test (n={count})"
    figure.suptitle(
...
def plot_spatial(
        ...
        rows.append(
            {
                ...
                "area_weighted_mean_rmse_reduction_mm_day": float(
                    np.sum(weights[support] * reduction[support]) / weights[support].sum()
                ),
            }
        )
    ...
    figure.suptitle("Exploratory IMD spatial comparison · 2020–2021", fontweight="semibold")
...
def evaluate_predictions(
    ...
    *,
    ...
    for method in METHOD_ORDER:
        ...
        frame["climatology_mse"] = engine.case_rmse(
            climatology, truth, weights
        ).reshape(-1) ** 2
        frame["pcc"] = [
            weighted_spatial_acc(
...
def summarize_by_lead(case_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = summary.merge(
        extra, on=["method", "lead_week"], validate="one_to_one"
    )
...
def paired_intervals_with_pcc(
    ...
    *,
    ...
    intervals = diagnostics.paired_intervals(
        case_metrics, initializations, smoke=smoke
    )
    ...
    for method, baseline_method in diagnostics.ALL_COMPARISONS:
        for scope, leads in diagnostics.LEAD_SCOPES.items():
            ...
            if not np.isfinite(model).all() or not np.isfinite(baseline_values).all():
                raise base.DataContractError("paired PCC bootstrap contains missing values")
            effect = float(model.mean() - baseline_values.mean())
            draws = (
                model[sampled].mean(axis=1)
                - baseline_values[sampled].mean(axis=1)
            )
            rows.append(
                {
                    "method": method,
    ...
    *,
    ...
    dataset = xr.Dataset(
        ...
        attrs={
            ...
            "inactive_weeks_exact_log_bias": list(
    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            else f"- W1–W{INACTIVE_LEAD_COUNT} remain exactly log-bias; "
    ...
    for candidate in CANDIDATES:
        ...
        lines.append(f"| {candidate.label} | {metadata['parameter_count']:,} | {mean_loss:.4f} |")
    lines.extend(
        [
            "",
            "## Validation decision",
...
def main() -> None:
    ...
    print("Loading aligned IMD and fitting the 2002–2017 calendar climatology...", flush=True)
    ...
    selection_record = {
        ...
        "checkpoint_sha256": {
            candidate.name: [
                record["checkpoint_sha256"] for record in training[candidate.name]["runs"]
            ]
            for candidate in CANDIDATES
        },
    }
    ...
    for method in METHOD_ORDER[2:]:
        if not np.array_equal(
            ...
            log_test[:, :INACTIVE_LEAD_COUNT],
            equal_nan=True,
        ):
            raise base.DataContractError(
                f"{method} violates inactive-week identity"
            )
    ...
    sources = (
        Path(__file__),
        HERE / "fuxi_imerg_a100_big_temporal.py",
    ...
    for artifact in sorted(path for path in output.rglob("*") if path.is_file()):
        if prediction_store in artifact.parents:
            continue
        manifest["artifacts"][str(artifact.relative_to(output))] = engine.sha256_file(artifact)
