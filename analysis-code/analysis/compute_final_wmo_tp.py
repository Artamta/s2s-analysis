...
for init_date in init_dates:
    ...
    try:
        ec_ds = xr.open_dataset(f'/storage/raj.ayush/s2s-forecast-data/ecmwf/data/sfc_cf_{init_str}.grib', engine='cfgrib', filter_by_keys={'shortName': 'tp'})
        ecmwf_dict[init_date] = apply_indian_subcontinent_bounding_box(ec_ds['tp']).rename({'latitude': 'lat', 'longitude': 'lon'})
    except: pass
    try:
        nc_ds = xr.open_dataset(f'/storage/raj.ayush/s2s-forecast-data/ncep/data/sfc_cf_{init_str}.grib', engine='cfgrib', filter_by_keys={'shortName': 'tp'})
...
for week_idx, (week_name, day_start, day_end) in enumerate(weeks):
    ...
    for init_date in init_dates:
        ...
        init_str = pd.to_datetime(init_date).strftime('%Y%m%d')
        try:
            path_end = f"/storage/raj.ayush/s2s-forecast-data/fuxi/output/{init_str}/member/00/{day_end:02d}.nc"
            path_start = f"/storage/raj.ayush/s2s-forecast-data/fuxi/output/{init_str}/member/00/{day_start-1:02d}.nc"
...
pd.DataFrame(regional_results).to_csv('/home/raj.ayush/s2s/s2s_anlysis/analysis-code/analysis/tp_regional_wmo.csv', index=False)
...
plt.tight_layout()
out_path = '/home/raj.ayush/s2s/s2s_anlysis/analysis-code/figures/verification/skill_horizon_tp_wmo.png'
