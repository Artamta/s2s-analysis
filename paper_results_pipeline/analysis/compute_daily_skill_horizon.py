...
for ld in range(1, 43): # Days 1 to 42
    ...
    for init_date in init_dates:
        ...
        init_str = pd.to_datetime(init_date).strftime('%Y%m%d')
        try:
            f_path = f"/storage/raj.ayush/fuxi-init-jfm-weekely/output/{init_str}/member/00/{ld:02d}.nc"
...
df.to_csv('/home/raj.ayush/s2s/s2s_anlysis/paper_results_pipeline/analysis/daily_skill_horizon_z500.csv', index=False)
...
plt.tight_layout()
out_path = '/home/raj.ayush/s2s/s2s_anlysis/paper_results_pipeline/figures/verification/skill_horizon_z500_daily.png'
