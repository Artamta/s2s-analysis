...
from utils.verification_wmo import get_cosine_latitude_weights, calc_wmo_acc, calc_wmo_rmse

print("Starting FINAL WMO-Compliant WEEKLY S2S Skill Horizon for Z500...")

# 1. Load ERA5
era_z_ds = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/era5/data/era5_pressure_500hpa.grib', engine='cfgrib')
...
for init_date in init_dates:
    ...
    try:
        ec_ds = xr.open_dataset(f'/storage/raj.ayush/benchmark(jfm)/ecmwf/data/pl_cf_{init_str}.grib', engine='cfgrib', filter_by_keys={'shortName': 'gh'})
        ecmwf_dict[init_date] = apply_indian_subcontinent_bounding_box(ec_ds['gh']).rename({'latitude': 'lat', 'longitude': 'lon'})
    except: pass
    try:
        nc_ds = xr.open_dataset(f'/storage/raj.ayush/benchmark(jfm)/ncep/data/pl_cf_{init_str}.grib', engine='cfgrib', filter_by_keys={'shortName': 'gh'})
...
for week_idx, (week_name, day_start, day_end) in enumerate(weeks):
    ...
    for init_date in init_dates:
        ...
        try:
            ...
            s_acc.append(calc_wmo_acc(s_d, e_z_week, era_z_clim, w_da))
            s_rmse.append(calc_wmo_rmse(s_d, e_z_week, w_da))
        except: pass
        ...
        for d in range(day_start, day_end + 1):
            f_path = f"/storage/raj.ayush/fuxi-init-jfm-weekely/output/{init_str}/member/00/{d:02d}.nc"
        ...
        if f_week_data:
            ...
            if 'lead_time' in f_d.dims: f_d = f_d.mean('lead_time')
            if 'member' in f_d.dims: f_d = f_d.mean('member')
            ...
            f_acc.append(calc_wmo_acc(f_d, e_z_week, era_z_clim, w_da))
            f_rmse.append(calc_wmo_rmse(f_d, e_z_week, w_da))
        ...
        if init_date in ecmwf_dict:
            try:
                ...
                e_acc.append(calc_wmo_acc(ec_d, e_z_week, era_z_clim, w_da))
                e_rmse.append(calc_wmo_rmse(ec_d, e_z_week, w_da))
            except: pass
        ...
        if init_date in ncep_dict:
            try:
                ...
                n_acc.append(calc_wmo_acc(nc_d, e_z_week, era_z_clim, w_da))
                n_rmse.append(calc_wmo_rmse(nc_d, e_z_week, w_da))
            except: pass
...
ax2.set_title('(b) S2S Weekly Horizon: 500hPa Geopotential Height (Z500) RMSE\n[WMO-Compliant Area-Weighted RMSE]', fontsize=14, fontweight='bold', pad=15)
ax2.legend(loc='upper left', fontsize=11)

plt.tight_layout()
out_path = '/home/raj.ayush/s2s/s2s_anlysis/paper_results_pipeline/figures/verification/skill_horizon_z500_wmo.png'
plt.savefig(out_path, bbox_inches='tight')
print(f"SUCCESS! WMO Horizon saved to {out_path}")
