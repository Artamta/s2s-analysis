...
This is the current clean-paper data flow before the full seasonal runners are
written.
...
Current model/truth roots registered in `code/s2s_paper/paths.py`:
...
`s2s_paper.grid.to_grid(field, GridSpec(...))`
...
`s2s_paper.grid.apply_region(field, region, dgrid=...)`
