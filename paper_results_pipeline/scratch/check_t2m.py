...
try:
    era_ds = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/era5/data/era5_surface.grib', engine='cfgrib', filter_by_keys={'shortName': '2t'})
    t2m_era = era_ds['t2m']
...
print("\n--- Checking ECMWF t2m ---")
ec_files = sorted(glob.glob('/storage/raj.ayush/benchmark(jfm)/ecmwf/data/sfc_cf_*.grib'))
for f in ec_files:
    try:
...
print("\n--- Checking NCEP t2m ---")
nc_files = sorted(glob.glob('/storage/raj.ayush/benchmark(jfm)/ncep/data/sfc_cf_*.grib'))
for f in nc_files:
    try:
