...
scripts in `../analysis/`, reorganised so each manuscript figure maps to exactly
one script.
...
```
 raw forecasts (/storage/.../s2s-forecast-data)   ARCO-ERA5 (cloud)
            │                                            │
            ▼                                            ▼
        compute/*  ───────────►  ../analysis/*.csv, *.nc, *.npz
                                          │
                                          ▼
        figures/*  ───────────►  ../paper/figs/fig*.pdf  +  ../analysis/tables_regional.tex
```
...
- **Intermediates** (skill tables, anomaly fields) are written to `../analysis/`.
- **Outputs** (figure PDFs/PNGs, the regional LaTeX table) go to `../paper/figs/`.
