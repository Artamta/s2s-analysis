...
OUT_MD = "/home/raj.ayush/s2s/s2s_anlysis/analysis-code/summary/model_data_dictionary.md"
...
def main():
    ...
    # 1. ERA5
    try:
        ds_era5 = xr.open_dataset("/storage/raj.ayush/s2s-forecast-data/era5/data/era5_surface_z500_20260101.nc")
    ...
    # 2. FuXi
    try:
        ds_fuxi = xr.open_dataset("/storage/raj.ayush/s2s-forecast-data/fuxi/data/20260101/input.nc")
    ...
    try:
        import zarr
        ds_spire = xr.open_zarr("/storage/raj.ayush/s2s-forecast-data/spire/spire_hindcast_jfm.zarr", group="mean_stddev", consolidated=False)
    ...
    try:
        ...
        dsets = cfgrib.open_datasets("/storage/raj.ayush/s2s-forecast-data/ecmwf/data/sfc_pf_20260101.grib")
    ...
    try:
        md = "### NCEP (S2S Archive)\n\n"
        dsets = cfgrib.open_datasets("/storage/raj.ayush/s2s-forecast-data/ncep/data/sfc_pf_20260101.grib")
        for i, ds in enumerate(dsets):
            md += f"#### GRIB Message Group {i+1}\n"
