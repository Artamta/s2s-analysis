...
class Config:
    # --- period identity ------------------------------------------------------
    season_label: str = 'JJAS2019'
    init_dates: tuple = (
        '2019-06-02', '2019-06-06', '2019-06-09', '2019-06-13', '2019-06-16',
        '2019-06-20', '2019-06-23', '2019-06-27', '2019-06-30', '2019-07-04',
        '2019-07-07', '2019-07-11', '2019-07-14', '2019-07-18', '2019-07-21',
        '2019-07-25', '2019-07-28', '2019-08-01', '2019-08-04', '2019-08-08',
        '2019-08-11', '2019-08-15', '2019-08-18', '2019-08-22', '2019-08-25',
        '2019-08-29', '2019-09-01', '2019-09-05', '2019-09-08', '2019-09-12',
        '2019-09-15', '2019-09-19', '2019-09-22', '2019-09-26', '2019-09-29'
    )
    valid_end: str = '2019-11-15'       # last ERA5 daily-truth date available
    valid_end_clim: str = '2019-11-15'  # last date used for clim-anomaly window
    grib_start: str = '2019-05-25'      # ERA5 GRIB start (before -> use dec25 patch)
    ...
    era5_daily_tp: str = '/home/raj.ayush/s2s/s2s_anlysis/paper/code/hfm/era5_daily_tp_2019.nc'
    era5_daily_t2m: str = ''
    era5_z500_nc: str = '/home/raj.ayush/s2s/s2s_anlysis/paper/code/hfm/era5_daily_z500_2019.nc'
    era5_surface_grib: str = ''
    dec25_patch: str = ''
...
# HFM systems. FuXi/ECMWF use a MODEL-OWN hindcast clima (files on disk).
BASE_SYSTEMS = ['FuXi', 'ECMWF']
...
def open_truth():
    ...
    t['z_raw'] = xr.open_dataset(CFG.era5_z500_nc)['z']  # already gpm and cropped
    t['dec25'] = None
    return t
...
def nearest_ecmwf_mmdd(init_str):
    import glob
    dt = pd.to_datetime(init_str)
    files = glob.glob('/storage/raj.ayush/All_Model_Data/models/ecmwf/data/tp_cf_*.grib')
    ec_mmdds = sorted(list({os.path.basename(f).split('_')[-1].split('.')[0] for f in files}))
    if not ec_mmdds:
        return None
    best_mmdd = None
    best_dist = 100
    for mmdd in ec_mmdds:
        m, d = int(mmdd[:2]), int(mmdd[2:])
        ec_dt = pd.Timestamp(year=dt.year, month=m, day=d)
        dist = (ec_dt - dt).days
...
def load_op(model, init_str, var, G):
    """ECMWF DataArray with dims (number, step, lat, lon) combining cf and pf; None if missing.
       TP stays CUMULATIVE (differenced later)."""
    best_mmdd = nearest_ecmwf_mmdd(init_str)
    if best_mmdd is None:
        return None
    
    base = '/storage/raj.ayush/All_Model_Data/models/ecmwf/data'
    time_sel = f"2019-{best_mmdd[:2]}-{best_mmdd[2:]}"
    
    try:
        if var == 'TP':
            pf_file = f'{base}/tp_pf_{best_mmdd}.grib'
            cf_file = f'{base}/tp_cf_{best_mmdd}.grib'
            
            ds_pf = xr.open_dataset(pf_file, filter_by_keys={'shortName': 'tp'}, **OPEN)
            ds_cf = xr.open_dataset(cf_file, filter_by_keys={'shortName': 'tp'}, **OPEN)
            
            pf = ds_pf['tp'].sel(time=time_sel)
            cf = ds_cf['tp'].sel(time=time_sel)
            
        elif var == 'Z500':
            pf_file = f'{base}/z500_pf_{best_mmdd}.grib'
            cf_file = f'{base}/z500_cf_{best_mmdd}.grib'
            
            ds_pf = xr.open_dataset(pf_file, filter_by_keys={'shortName': 'gh'}, **OPEN)
            ds_cf = xr.open_dataset(cf_file, filter_by_keys={'shortName': 'gh'}, **OPEN)
            
            pf = ds_pf['gh'].sel(time=time_sel)
            cf = ds_cf['gh'].sel(time=time_sel)
            if 'isobaricInhPa' in pf.dims:
                pf = pf.sel(isobaricInhPa=500)
            if 'isobaricInhPa' in cf.dims:
                cf = cf.sel(isobaricInhPa=500)
        else:
            return None
            
        # Combine control forecast (member 0) and 10 perturbed forecasts (members 1-10)
        cf_expanded = cf.expand_dims(number=[0])
        combined = xr.concat([cf_expanded, pf], dim='number')
        return crop_box(combined)
    except Exception as e:
        print(f"  [{model}] {var} load fail {init_str} (mmdd={best_mmdd}): {e}", flush=True)
    return None
