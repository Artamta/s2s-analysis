...
def load_spire(init):
    """{var: (mean_da, stddev_da)} with a `step` dim; None if unavailable.
       Both mean and stddev come from 'mean_stddev' (absolute ensemble mean +
       spread). The anomaly group is NOT used: its ERA5 clim reference differs
       from our ERA5 clim by ~17 gpm (Z500) / 0.2 mm/day (TP), inflating
       RMSE/MSSS. Scoring subtracts ERA5 clim internally for anomaly metrics."""
    try:
        ds = xr.open_zarr(CFG.spire_zarr, group='mean_stddev').sel(reference_time=init)
    except Exception as e:
        print(f"  [SPIRE] open fail {init}: {e}", flush=True)
        return None
    out = {}
    try:
        out['TP'] = (crop_box(ds['precipitation_amount']),
                     crop_box(ds['precipitation_amount_stddev']))
    except Exception:
        pass
    try:
        ...
        pass
    try:
        out['T2M'] = (crop_box(ds['air_temperature']),
                      crop_box(ds['air_temperature_stddev']))
    except Exception:
        pass
    return out or None
