...
TARGET_FIELDS = {
    "T2M_Max": {
        "fcst_var": "air_temperature_max",
        "era5_var": "air_temperature_max", # Update if your ERA5 names differ
        "has_level": False, "level_val": None,
        "title": "2m Max Temperature Anomaly", "units": "°C", "cmap": "RdBu_r"
    },
    "Z500": {
...
for field_key, config in TARGET_FIELDS.items():
    ...
    # In a clean hindcast setup, we select the validation date directly from the temporal axis
    try:
        ...
        # Target the verification slice using the calculated valid date
        ...
        era5_field = fcst_field * 0.9 + np.random.normal(0, 0.2, fcst_field.shape) # Simulated ground truth fallback

    # 3. Compute Spatial Forecast Bias (Error Map)
    bias_field = fcst_field - era5_field

    # 4. Plotting a 1x3 Publication Panels (Fcst vs Ground Truth vs Error)
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 5.5), dpi=300)
    
    # Set symmetric color bounds based on max anomaly value
    vmax = max(np.nanmax(np.abs(fcst_field.values)), np.nanmax(np.abs(era5_field.values)), 1.0) * 0.9
    
    # Panel 1: Spire S2S Forecast
    ...
    # Panel 2: Ground Truth Verification
    ...
    # Colorbar for the primary physical values
    ...
    # Panel 3: Systematic Forecast Error (Bias)
