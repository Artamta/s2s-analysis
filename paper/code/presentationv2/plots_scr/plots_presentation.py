...
def fig08_scatter_r2_mae(det):
    ...
    scats_models = ['SPIRE', 'FuXi', 'ECMWF', 'MME']
    vars_ = ['TP', 'Z500']

    fig, axes = plt.subplots(len(vars_), len(scats_models),
                             figsize=(14, 6.5), sharey='row', sharex='row')
...
def fig09_scorecard(det, prob):
    ...
    if nrows == 1:
        axes = [axes]
    fig.suptitle('Scorecard — All India (mean over 13 inits)', fontsize=12, y=1.01)
...
def fig10_daily_pcc(det):
    ...
    fig.suptitle('Daily Pattern Correlation — All India (mean over 13 inits)', fontsize=12, y=1.01)
