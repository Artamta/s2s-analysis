#!/usr/bin/env python3
"""Build paper-ready result tables and comparison figures.

This script consumes the validated weekly metric CSVs and writes a clean
``paper_results`` directory. The lead axis is the standard S2S weekly windows
...
PUBLICATION_RUNS = (
    ...
    RunConfig(
        ...
        paper_case="jfm2026_daily_spire",
        paper_title="JFM2026 Daily Init Dates With SPIRE",
...
def add_run_columns(df: pd.DataFrame, config: RunConfig) -> pd.DataFrame:
    if df.empty:
        return df
    out = df.copy()
    out.insert(0, "paper_case", config.paper_case)
    out.insert(1, "paper_title", config.paper_title)
    out.insert(2, "paper_note", config.note)
    return out
...
def write_grid_scatter_sample(output_root: Path, data_dir: Path, max_points_per_model: int) -> str | None:
    """Write a compact India-grid sample for paper scatter reproduction."""
    ...
    for config in PUBLICATION_RUNS:
        ...
        for (_, _, variable, model), group in table.groupby(
            ["paper_case", "paper_title", "variable", "model"], sort=True
        ):
...
def plot_deterministic_figures(det: pd.DataFrame, fig_dir: Path) -> list[dict[str, str]]:
    ...
    if det.empty:
        return records
    for (paper_case, paper_title, variable, region), group in det.groupby(
        ["paper_case", "paper_title", "variable", "region"], sort=True
    ):
        unit = str(group["unit"].dropna().iloc[0]) if "unit" in group and group["unit"].notna().any() else ""
        title = f"{paper_title}: {label_variable(variable)} | {label_region(region)}"
        out_base = fig_dir / "lead_skill" / "deterministic" / (
            f"{safe_name(paper_case)}_{safe_name(region)}_{safe_name(variable)}_deterministic_lead_metrics"
        )
...
def plot_probabilistic_figures(prob: pd.DataFrame, fig_dir: Path) -> list[dict[str, str]]:
    ...
    if prob.empty:
        return records
    for (paper_case, paper_title, variable, region), group in prob.groupby(
        ["paper_case", "paper_title", "variable", "region"], sort=True
    ):
        unit = str(group["unit"].dropna().iloc[0]) if "unit" in group and group["unit"].notna().any() else ""
        title = f"{paper_title}: {label_variable(variable)} | {label_region(region)}"
        out_base = fig_dir / "lead_skill" / "probabilistic" / (
            f"{safe_name(paper_case)}_{safe_name(region)}_{safe_name(variable)}_probabilistic_lead_metrics"
        )
...
def plot_brier_figures(brier: pd.DataFrame, fig_dir: Path) -> list[dict[str, str]]:
    ...
    if brier.empty:
        return records
    group_cols = ["paper_case", "paper_title", "event", "threshold", "region"]
    for keys, group in brier.groupby(group_cols, sort=True):
        paper_case, paper_title, event, threshold, region = keys
        title = f"{paper_title}: {event} | {label_region(region)}"
        out_base = fig_dir / "lead_skill" / "brier" / (
            f"{safe_name(paper_case)}_{safe_name(region)}_{safe_name(event)}_brier_lead_metrics"
        )
...
def plot_area_scatter_figures(scatter_area: pd.DataFrame, fig_dir: Path, max_points_per_model: int) -> list[dict[str, str]]:
    ...
    if scatter_area.empty:
        return records
    for (paper_case, paper_title, variable, region), group in scatter_area.groupby(
        ["paper_case", "paper_title", "variable", "region"], sort=True
    ):
        unit = str(group["unit"].dropna().iloc[0]) if "unit" in group and group["unit"].notna().any() else ""
        title = f"{paper_title}: {label_variable(variable)} area-mean scatter | {label_region(region)}"
        if unit:
            title += f" ({unit})"
        out_base = fig_dir / "scatter" / "area_mean" / (
            f"{safe_name(paper_case)}_{safe_name(region)}_{safe_name(variable)}_area_scatter_by_model"
        )
...
def plot_grid_scatter_figures(output_root: Path, fig_dir: Path, max_points_per_model: int) -> list[dict[str, str]]:
    ...
    for config in PUBLICATION_RUNS:
        ...
        for variable, group in table.groupby("variable", sort=True):
            unit = str(group["unit"].dropna().iloc[0]) if "unit" in group and group["unit"].notna().any() else ""
            title = f"{config.paper_title}: {label_variable(variable)} grid-point scatter | India grid"
            if unit:
                title += f" ({unit})"
            out_base = fig_dir / "scatter" / "grid_sample" / (
                f"{safe_name(config.paper_case)}_{safe_name(variable)}_grid_scatter_by_model"
            )
            paths = plot_scatter_facets(group, title, out_base, max_points_per_model=max_points_per_model, pdf=False)
...
def write_readme(paper_dir: Path, data_outputs: dict[str, str], figure_count: int) -> None:
    lines = [
        "# S2S Paper Results",
        "",
        "This directory contains paper-facing products generated from the validated final-paper metric runs.",
        "",
    ...
    for name, path in sorted(data_outputs.items()):
        rel = Path(path).relative_to(paper_dir)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--paper-dir-name", default="paper_results")
...
def main() -> int:
    ...
    paper_dir = args.output_root / args.paper_dir_name
    ...
    (paper_dir / "manifest.json").write_text(json.dumps(json_manifest, indent=2))
    write_readme(paper_dir, data_outputs, len(figure_manifest))

    print(f"Paper results: {paper_dir}")
