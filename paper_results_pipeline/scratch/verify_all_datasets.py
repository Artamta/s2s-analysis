...
try:
    era_sfc = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/era5/data/era5_surface.grib', engine='cfgrib', backend_kwargs={'indexpath': ''})
    ...
    print(f"  - Location: /storage/raj.ayush/benchmark(jfm)/era5/data/era5_surface.grib")
...
try:
    # Check one day
    fx_files = sorted(glob.glob('/storage/raj.ayush/fuxi-init-jfm-weekely/output/20260101/member/00/*.nc'))
    if fx_files:
        fx = xr.open_dataset(fx_files[-1])
        lat_res = abs(fx.lat.values[0] - fx.lat.values[1])
        lon_res = abs(fx.lon.values[0] - fx.lon.values[1])
        print(f"  - Location: /storage/raj.ayush/fuxi-init-jfm-weekely/output/[init_date]/member/00/[day].nc")
...
try:
    ...
    print(f"  - Location: /storage/raj.ayush/spire-hindecast-weekely-initialized/spire_hindcast_jfm.zarr")
...
print("\n4. ECMWF (Operational)")
try:
    ec_cf = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/ecmwf/data/sfc_new_cf_20260101.grib', engine='cfgrib', backend_kwargs={'indexpath': ''})
    ec_pf = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/ecmwf/data/sfc_new_pf_20260101.grib', engine='cfgrib', backend_kwargs={'indexpath': ''})
    lat_res = abs(ec_cf.latitude.values[0] - ec_cf.latitude.values[1])
    lon_res = abs(ec_cf.longitude.values[0] - ec_cf.longitude.values[1])
    print(f"  - Location: /storage/raj.ayush/benchmark(jfm)/ecmwf/data/sfc_new_[cf/pf]_[date].grib")
...
print("\n5. NCEP (Operational)")
try:
    nc_cf = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/ncep/data/sfc_cf_20260101.grib', engine='cfgrib', backend_kwargs={'indexpath': ''})
    nc_pf = xr.open_dataset('/storage/raj.ayush/benchmark(jfm)/ncep/data/sfc_pf_20260101.grib', engine='cfgrib', backend_kwargs={'indexpath': ''})
    lat_res = abs(nc_cf.latitude.values[0] - nc_cf.latitude.values[1])
    lon_res = abs(nc_cf.longitude.values[0] - nc_cf.longitude.values[1])
    print(f"  - Location: /storage/raj.ayush/benchmark(jfm)/ncep/data/sfc_[cf/pf]_[date].grib")
