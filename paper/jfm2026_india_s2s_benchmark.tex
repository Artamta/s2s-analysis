...
We evaluate four S2S forecast systems spanning two dynamical NWP ensembles and two
AI-based systems. Table~\ref{tab:models} summarizes their configurations as delivered
for the JFM~2026 period; the two dynamical systems are obtained from the WWRP/WCRP S2S
database \citep{vitart2017}, while the two AI systems were received directly from
...
\begin{table}[t]
  ...
  \begin{tabular}{llccc}
    \toprule
    System & Type & Native res. & Members & Horizon \\
    \midrule
    Spire AI-S2S    & AI (data-driven)    & $0.5^{\circ}$          & mean\,$+$\,s.d.     & 46\,d \\
    FuXi-S2S        & AI (data-driven)    & $1.5^{\circ}$          & 1\,CF\,$+$\,10\,PF  & 42\,d \\
    ECMWF ext-range & Dynamical, coupled  & Tco319 ($\sim$36\,km)  & 100\,PF             & 46\,d \\
    NCEP CFSv2      & Dynamical, coupled  & T126 ($\sim$100\,km)   & 15\,PF              & 44\,d \\
    \bottomrule
  \end{tabular}
\end{table}
...
consistent: precipitation ACC falls below 0.5 by week~3 for every system
(Table~\ref{tab:acc}). The Z500 skill horizon is two to three weeks
longer, also in line with global benchmarks \citep{vitart2017,rasp2024}.
The principal departure from the historical picture is that the leading
system is an AI model (SPIRE) rather than ECMWF; with 13 initializations
...
(Tables~\ref{tab:reg_pcc}--\ref{tab:reg_bias}).
...
\textbf{Additional systems.} Adding FourCastNet-3, the Deep Learning
Earth System Model \citep{cresswellclay2024}, and the IMD Extended
Range Prediction and Advisory System (ERPaS;
\citealt{pattanaik2019}) would enable a direct operational comparison
with the system India's forecasters actually use. ERPaS in particular
issues tercile-category probabilistic guidance and is directly
comparable to the CRPSS and reliability diagnostics developed here.
...
We presented a real-time subseasonal benchmark over India for
January--March 2026, evaluating two AI-based systems (SPIRE AI-S2S,
FuXi-S2S) against two operational dynamical systems (ECMWF extended
range, NCEP CFSv2) across 13 weekly initializations and six lead weeks,
for precipitation and 500-hPa geopotential height. Forecasts were
verified against ERA5 using cosine-latitude-weighted ACC, RMSE, bias,
CRPSS, and spread--skill ratio, with bootstrap confidence intervals and
land masking over the four IMD homogeneous regions. The main conclusions
are as follows.
...
Within the limits of a single season and 13 initializations, these
results establish that current AI S2S systems can match or exceed
operational dynamical models for India---SPIRE leads for precipitation
and is tied with ECMWF for Z500---and that calibrated probabilistic
guidance is where the AI systems most visibly diverge from each other.
The verification framework, code, and output data are archived to
support replication and extension as additional seasons and systems
become available.
