#!/usr/bin/env python3
"""Stage one no-leak ARCO-ERA5 initial condition on NeuralGCM's native grid."""
...
def verify_checkpoint(config: dict[str, Any]) -> tuple[Path, str]:
    ...
    if not checkpoint.is_file():
        raise FileNotFoundError(
            f"checkpoint is not provisioned at {checkpoint}; run the controlled "
            "checkpoint copy/download step first"
        )
    ...
    if digest != config["model"]["checkpoint_sha256"]:
        raise ValueError(
            ...
            f"expected {config['model']['checkpoint_sha256']}, found {digest}"
        )
...
def stage(config: dict[str, Any], force: bool) -> tuple[Path, Path]:
    ...
    manifest_path = pilot_contract.storage_path(config, "input_manifest")
    ...
    if (input_path.exists() or manifest_path.exists()) and not force:
        raise RuntimeError(
            "partial or invalid staged IC exists; inspect it and rerun with --force "
    ...
    forcing_time = np.datetime64(
        config["initial_conditions"]["forcing_source_time"]
    )
    source = xr.open_zarr(
        config["source"]["url"],
        chunks=None,
    ...
    try:
        ...
        for variable in model.forcing_variables:
            np.testing.assert_allclose(
                ...
                atol=0.0,
                equal_nan=True,
                err_msg=f"24-hour shift failed for {variable}",
            )
    ...
    if native.sizes.get("time") != 1:
        raise ValueError(f"expected one IC time, found {native.sizes.get('time')}")
    if native.sizes.get("longitude") != 128 or native.sizes.get("latitude") != 64:
        raise ValueError(
            "unexpected NeuralGCM native grid: "
            f"longitude={native.sizes.get('longitude')} "
            f"latitude={native.sizes.get('latitude')}"
        )
    expected_levels = np.asarray(model.data_coords.vertical.centers)
    if not np.array_equal(native.level.values, expected_levels):
        raise ValueError("staged pressure levels differ from the checkpoint")
    ...
    request_contract = {
        ...
        "atmosphere_source_time": config["initial_conditions"][
            "atmosphere_source_time"
        ],
        ...
        "target_grid": config["model"]["native_grid"],
    ...
    native.attrs.update(
        {
            ...
            "forcing_source_time": config["initial_conditions"][
                "forcing_source_time"
            ],
            ...
            "paper_status": "smoke input only; not for skill scores",
        }
    )
    ...
    with xr.open_dataset(temporary) as check:
        if set(check.data_vars) != set(variables):
            raise ValueError("written IC variables differ from the checkpoint contract")
        check.load()
