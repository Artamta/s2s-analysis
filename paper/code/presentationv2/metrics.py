...
def acc(f, o, clim, w, clim_o=None):
    """WMO anomaly pattern correlation (spatially centred, area-weighted).
       ...
       V2: `clim` is the FORECAST climatology and `clim_o` (if given) the
       OBSERVATION climatology, so the forecast and obs anomalies may be taken
       against DIFFERENT baselines (model-own vs ERA5). When clim_o is None the
       classic single-clim behaviour is recovered."""
...
def std_ratio(f, o, clim, w, clim_o=None):
    """Forecast-vs-obs anomaly spatial-std ratio (amplitude fidelity).
       V2: separate forecast vs observation climatology (see acc)."""
    co = clim if clim_o is None else clim_o
