#!/usr/bin/env python3
"""Shared contracts for reproducible FCN3 plus AFNOv2 inference."""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if len(config["model"]["seeds"]) != config["model"]["members"]:
        raise ValueError("member seed count does not match member count")
    if len(set(config["model"]["seeds"])) != config["model"]["members"]:
...
def product_paths(config: dict[str, Any], init_date: str) -> dict[str, Path]:
    root = (
        Path(config["storage"]["root"])
        / "fcn3"
        / config["model"]["run_label"]
    )
    year = init_date[:4]
    stamp = init_date.replace("-", "")
    return {
        "root": root,
        "stage": root / "initial-conditions" / year / f"{stamp}.nc",
        "stage_manifest": root / "initial-conditions" / year / f"{stamp}.json",
        "output": root / "forecasts" / year / f"{stamp}.nc",
        "manifest": root / "manifests" / year / f"{stamp}.json",
        "failed": root / "failures" / year / f"{stamp}.json",
...
def target_coordinates(config: dict[str, Any]) -> tuple[np.ndarray, np.ndarray]:
    ...
    latitude = np.arange(lat_start, lat_stop + lat_step / 2, lat_step)
    longitude = np.arange(lon_start, lon_stop + lon_step / 2, lon_step)
...
def conservative_weights(
    ...
    """Return separable spherical-cell overlap weights for target area means."""
    ...
    for row, center in enumerate(target_lat):
        lo, hi = center - target_spacing / 2, center + target_spacing / 2
    ...
    digest = hashlib.sha256(lat_weights.tobytes() + lon_weights.tobytes()).hexdigest()
    return lat_weights, lon_weights, digest
